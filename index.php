...
<!-- Carousel  -->
  ...
  <section class="carousel-section" id="slider">
   ...
   <div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel">
       ...
       <?php $a=0; while($slider=$sqlcatab->fetch()){?>
         <div class="carousel-item <?php if($a==1) echo "active"?>">
           <img class="d-block w-100" src="admin/vues/sliderImages/<?php echo $slider['image_name']?>" alt="First slide">
           <div class="carousel-caption d-none d-md-block">
               <h1 class="display-5"><?php echo $slider['title']?></h1>
               <p><?php echo $slider['desc_image']?></p>
             </div>
         </div>
         <?php $a++;}  ?>
         </div>
 ...
 <section class="py-5 white-section" id="introduction" >
   <div class="container">
     <h1 class="display-4 title"><?php echo $row['nom_hotel'] ?></h1>
     
   </div>

   <div class="container"> 
       
   <?php echo $row['desc_hotel'] ?> 
      </div>       
   </section>    
   ...
   <section class="py-5 colored-section" id="grid" >
           ...
           <?php $sqlroom=$db->query("SELECT * ,rooms.id as id_room FROM rooms inner join cotegorie on rooms.fkcategorie=cotegorie.id");
while($room=$sqlroom->fetch()){

  $simg=$db->query("SELECT * from  images_room  where images_room.idroom='".$room['id_room']."' LIMIT 1");
  $rim=$simg->fetch();
  ?>
               ...
               <div class="box container text-center">
                 <a href="detailroom.php?room=<?php echo $room['id_room']?>">
                  <img class='img mb-4' src='admin/vues/roomImages/<?php echo $rim['images']?>' alt='Image Error' width='350px' height='200px'>
                     
                  <center><h3 style="color:black"><?php echo $room['nom_categorie']?></h3></center>
                  </a>
                   <p class="mb-4"><?php echo  substr($room['resume'], 0, 100); ?></p>
                    <label for=""><a href="detailroom.php?room=<?php echo $room['id_room']?>">Détails</a></label>  
               </div>
               </div>
